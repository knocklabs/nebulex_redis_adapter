defmodule Knock.Nebulex.Adapters.Redis.Cluster.ConfigManager do
  @moduledoc false
  ...
  alias Knock.Nebulex.Adapters.Redis.{Cluster, ErrorFormatter}
  alias Knock.Nebulex.Adapters.Redis.Cluster.PoolSupervisor
  ...
  @spec start_link({Knock.Nebulex.Adapter.adapter_meta(), keyword()}) :: GenServer.on_start()
  def start_link({adapter_meta, opts}) do
    name = camelize_and_concat([adapter_meta.name, ConfigManager])
  ...
  def init({adapter_meta, opts}) do
    state = %__MODULE__{
      ...
      opts: opts,
      dynamic_sup: camelize_and_concat([adapter_meta.name, DynamicSupervisor])
    }
  ...
  def terminate(_reason, %__MODULE__{adapter_meta: meta, dynamic_sup: sup, running_shards: lst}) do
    ...
    :ok = Cluster.del_status_key(meta.name)
  ...
  defp do_setup_shards(
    ...
    # Lock the cluster
    :ok = Cluster.put_status(name, :locked)
    ...
    case configure_shards(state) do
      ...
      {:error, reason} ->
        ...
        # Set cluster status to error
        :ok = Cluster.put_status(name, :error)
  ...
  defp configure_shards(%__MODULE__{
    ...
    Telemetry.span(adapter_meta.telemetry_prefix ++ [:redis_cluster, :setup], metadata, fn ->
      case configure_shards(adapter_meta, dynamic_sup, running_shards, opts) do
        {:ok, _} = ok ->
          {ok, %{metadata | status: :ok, reason: :succeeded}}

        {:error, reason} ->
          # Wrap up the error with the Telemetry metadata
          error =
  ...
  defp stop_running_shards(cluster_shards_tab, dynamic_sup, running_shards) do
    ...
    true = :ets.delete_all_objects(cluster_shards_tab)
